...
export default function ConfigWindow() {
  ...
  const handleStartHotkeyCapture = async () => {
    ...
    try {
      ...
      const isWin = typeof window !== 'undefined' && window.electron?.platform === 'win32';
      setHotkeyError(
        isWin
          ? 'Could not start capture. Close other apps that might use the same hotkey, then try again.'
          : 'Could not start capture. Make sure Accessibility permission is granted.',
      );
    }
  };
  ...
  const isWindowsPlatform = typeof window !== 'undefined' && window.electron?.platform === 'win32';
  const isMacPlatform = typeof window !== 'undefined' && window.electron?.platform === 'darwin';

  const permissionsAllOk = permissions
    && permissions.microphone === 'granted'
    && (isWindowsPlatform
      ? permissions.pushToTalkHotkeyActive
      : permissions.screen === 'granted' && permissions.accessibility === 'granted');

  const hotkeyRowState: MacPermissionState = isWindowsPlatform
    ? (permissions?.pushToTalkHotkeyActive ? 'granted' : 'unknown')
    : (permissions?.accessibility ?? 'unknown');

  const osName = isWindowsPlatform ? 'Windows' : 'macOS';
  const settingsAppName = isWindowsPlatform ? 'Settings' : 'System Settings';
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: THEME.surface.base, color: THEME.text.primary }}>
      ...
      >
        {/* Desktop permissions (macOS vs Windows) */}
        <Card>
          <CardHeader
            title={isWindowsPlatform ? 'Windows permissions' : 'macOS Permissions'}
            description={
              isWindowsPlatform
                ? `Sally needs microphone access and a working global hotkey. Use the buttons below to open ${settingsAppName} or retry registration. Screen capture may prompt the first time you use vision features.`
                : `Sally needs three system permissions to work end to end. Granting them takes you straight to the right pane in ${settingsAppName}.`
            }
            indicator={permissionsAllOk ? 'green' : 'gray'}
          />
          <PermissionRow
            label="Microphone"
          ...
          <PermissionRow
            ...
            description={
              isWindowsPlatform
                ? "Used when you ask what you're looking at. Windows may prompt the first time Sally captures the screen."
                : "Lets Sally see your screen when you ask 'what am I looking at?'."
            }
            state={permissions?.screen ?? 'unknown'}
            secondaryAction={{ label: 'Open Settings', onClick: () => handleOpenSettingsPane('screen') }}
          />
          <PermissionRow
            label={isWindowsPlatform ? `Global hotkey (${pushToTalkKeyLabel})` : `Accessibility (${pushToTalkKeyLabel} hotkey)`}
            description={
              isWindowsPlatform
                ? `Sally listens for ${pushToTalkKeyLabel} system-wide via uIOhook.${permissions?.pushToTalkHotkeyActive ? ' Hotkey is active.' : ' If this stays inactive, try Retry hotkey or restart Sally.'}`
                : `Required so Sally can listen for ${pushToTalkKeyLabel} from any app.${permissions?.pushToTalkHotkeyActive ? ' Hotkey is active.' : ''}`
            }
            state={hotkeyRowState}
            primaryAction={
              isWindowsPlatform
                ? { label: 'Retry hotkey', onClick: handlePromptAccessibility }
                : { label: 'Grant', onClick: handlePromptAccessibility }
            }
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 12, color: THEME.text.secondary }}>
                  {openAtLogin
                    ? `${osName} will launch Sally automatically when you sign in.`
        ...
        <Card>
          <CardHeader
            ...
            description={
              isMacPlatform
                ? 'Hold this key (or combination of keys) anywhere on your Mac to talk to Sally.'
                : 'Hold this key (or combination of keys) anywhere on your computer to talk to Sally.'
            }
