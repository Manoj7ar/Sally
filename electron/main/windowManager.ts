...
import { applyFloatingChrome, baseWebPreferences, browserWindowChrome, configWindowChrome } from './platform/windowChrome.js';
...
class WindowManager {
  ...
  showConfigWindow(): BrowserWindow {
    ...
    this.configWindow = new BrowserWindow({
      ...
      show: false,
      webPreferences: baseWebPreferences(this.getPreloadPath()),
      ...configWindowChrome(),
    });
  ...
  showSallyBar(): BrowserWindow {
    ...
    applyFloatingChrome(this.sallyBarWindow);
  ...
  private ensureBorderOverlayWindow(bounds: { x: number; y: number; width: number; height: number }): void {
    ...
    applyFloatingChrome(this.borderOverlayWindow);
